...
def update_project(project_id: int, name: str, color: str, 
                   pomodoro_enabled: bool, afk_enabled: bool):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE projects 
                SET name = %s, color = %s, pomodoro_enabled = %s, afk_enabled = %s
                WHERE id = %s
            """, (name, color, pomodoro_enabled, afk_enabled, project_id))
